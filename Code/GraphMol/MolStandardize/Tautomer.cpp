...
std::vector<ROMOL_SPTR> TautomerEnumerator::enumerate(const ROMol &mol, TautomerCatalog *tautcat) {
	...
	const std::vector<TautomerTransform> &transforms = tautparams->getTransforms();
	...
	std::map< std::string, boost::shared_ptr<ROMol> > kekulized_mols = {{smi, kekulized}};
...
//					if (search != kekulized_mols.end()
//					for (const auto &mol : kekulized_mols) {
//							if (mol.first == tautomer.first) {
//								std::cout << mol.first << std::endl;
//							}
//					std::cout << MolToSmiles(*transform.Mol) << std::endl;
					std::vector< MatchVectType > matches;
					unsigned int matched = SubstructMatch( *(kmol->second), *(transform.Mol), matches );
					std::string name;
					...
					std::cout << "transform mol: " << MolToSmiles(*(transform.Mol)) << std::endl;
					...
					for (const auto &match : matches) {
						...
						first->setNumExplicitHs( std::max((unsigned int) 0, first->getTotalNumHs() - 1) );
...
//		for (auto &bond : (tmp)->getBonds()) {
		for (size_t i = 0; i < tmp->getNumBonds(); ++i) {		
			...
			if (bond->getBondType() == 2 && bond->getStereo() > Bond::BondStereo::STEREOANY) {
				...
				for (auto &other_tautomer : tautomers) {
					...
					if ( !( other_tmp->getBondBetweenAtoms(begin, end)->getBondType() == 2 )) {
						...
						while (beg != end) {
							...
							if (bonddir == Bond::BondDir::ENDUPRIGHT || bonddir == Bond::BondDir::ENDDOWNRIGHT ) {
